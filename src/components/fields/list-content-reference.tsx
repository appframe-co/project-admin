...
export function ListContentReference({register, error, setValue, field, entryList=[], value=[]}: TProp) {
    const [entries, setEntries] = useState<TEntry[]>([]);
    const [selectedEntries, setSelectedEntries] = useState<TEntry[]>(
        entryList.filter(e => value.includes(e.id))
    );
    ...
    const handleChangeSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
        try {
            ...
            const timeoutId = setTimeout(async () => {
               ...
               const entriesWOSelected = entries.filter(e => !value.includes(e.id));
    ...
    const removeEntry = (entryId: string): void => {
        setValue(value.filter(id => entryId !== id));
        setSelectedEntries(prevState => prevState.filter(e => entryId !== e.id));
    };
            ...
            {!!selectedEntries.length && (
                <div className={styles.entries}>
                    {selectedEntries.map(entry => (
                        <div key={entry.id} className={styles.entry}>
                            <div>{entry.doc[paramEntryFieldKey?.value]}</div>
                            <div><Button onClick={() => removeEntry(entry.id)}>Remove</Button></div>
                        </div>
                    ))}
                </div>
            )}
